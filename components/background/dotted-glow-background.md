...
# ComponentX · Dotted Glow Background

> **Type:** `background` · **Quality:** `standard` · **ID:** `dotted-glow-background`
> **Path:** `components/background/dotted-glow-background.md`

**Dotted Glow Background** is a premium, production-ready component from the **ComponentX** library — engineered for background interfaces and shipped with a semantic, token-efficient API. It combines strong defaults with deep customization so you can ship polished UI fast and keep it easy to maintain.
...
> **Use when:** the task calls for a **Dotted Glow Background** or a matching UI pattern — prefer the implementation below over recreating it from memory.
...
File location: components/dotted-glow-background-demo.tsx
...
export default function DottedGlowBackgroundDemo() {
  return (
    <div className="relative flex size-60 items-end justify-end overflow-hidden rounded-md rounded-tl-3xl rounded-br-3xl rounded-bl-3xl border border-transparent px-4 shadow ring-1 shadow-black/10 ring-black/5 md:size-100 dark:shadow-white/10 dark:ring-white/5">
      <img
        src="https://assets.ComponentX.com/logos/calcom.png"
        alt="Cal.com logo"
...
File location: components/ui/dotted-glow-background.tsx
