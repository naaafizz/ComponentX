...
type: "form"
quality: "showcase"
tags: ["animation", "component", "data", "loading", "shader", "text", "webgl"]
load_policy: "Load only after this component is selected from CATALOG.md."
---

# ComponentX · Temple Night

> **Type:** `form` · **Quality:** `showcase` · **ID:** `temple-night`
> **Path:** `components/form/temple-night.md`

**Temple Night** is a premium, production-ready component from the **ComponentX** library — engineered for form interfaces and shipped with a semantic, token-efficient API. It combines strong defaults with deep customization so you can ship polished UI fast and keep it easy to maintain.
...
- **Pattern coverage** — includes `animation`, `data`, `loading` workflows.
- **Pattern coverage** — includes `shader`, `text`, `webgl` workflows.
- **Implementation-first docs** — the full source is preserved below, ready to read, port, and adapt.

> **Use when:** the task calls for a **Temple Night** or a matching UI pattern — prefer the implementation below over recreating it from memory.
...
description: "Build Temple Night from its verified authored source using Three.js r149, including the complete renderer, interactions, and required assets. Use when Codex needs to implement, port, or adapt this effect without requiring the ComponentX package or reconstructing the visual from an approximation."
---
...
One procedural world engine pointed at four places: Kage’s Kyoto mountain temple after dark, Yosemite Valley under alpenglow, a Presidio park above the Golden Gate, and Lake Louise at dusk — same rig, same foreground plates, same bloom pipeline.
...
- Three.js r149 procedural world construction
- Manual render-target post-processing with the exact 16-pass bloom/composite pipeline
- Instanced architecture and foliage, particles, fog, rain, mist, and pointer wisps
...
2. Pin Three.js r149 and transfer only the complete authored world sections from the Kage source.
3. Build the mountain temple, courtyard, torii, lanterns, maple canopy, moon, terrain, rain, leaves, fog, and lights in source order.
4. Reproduce the scene target, multilevel ping-pong bloom chain, and final composite without replacing it with a post library.
5. Preserve the authored hero camera, pointer parallax, world-space wisps, adaptive quality, and visibility behavior.
6. Dispose post targets, geometries, materials, textures, observers, listeners, and the renderer.
...
```tsx
import { TempleNightScene } from "./effects/temple-night/TempleNightScene";
import "./effects/temple-night/styles.css";

export function Scene() {
  return <div className="effect-frame"><TempleNightScene /></div>;
}
```
...
renderPost(); // exact downsample, blur, upsample, and composite sequence
...
- Assets: No external scene assets
- **renderer** (host): Three.js r149
- **variant** (choice): temple-night | yosemite-sunset | presidio-sunset | lake-louise
- **scene** (fixed): Procedural temple, valley, headland, or lake world
- **post** (fixed): 16 authored passes
- **interaction** (pointer): Parallax + motes
- **pixelRatio** (adaptive): ≤ 1.8
- **assets** (fixed): None
...
# Temple Night — Temple Night — Complete source

Component: `TempleNightScene`
Variant: **Temple Night** (`temple-night`)
Runtime: Three.js r149
Source revision: `4399487d2fb42bce39c7b032fbbb50d230bf4f0b`
...
```tsx
import { TempleNightScene } from "@componentx/threeui";
...
Role: component · 151 lines · 5511 bytes · SHA-256 `3974d55c0bebd92bc3bb3a3a32c0fd60e01ef0a60da8d310e5265018269b4d0a`
...
Role: renderer-source · 2536 lines · 124050 bytes · SHA-256 `9da1d88d83877141f61476d97055abe89aa3c4c008e9c098af5a4b552bed15b2`
