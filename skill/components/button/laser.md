...
> **Type:** `button` · **Quality:** `showcase` · **ID:** `laser`
> **Path:** `components/button/laser.md`

**Laser** is a premium, production-ready component from the **ComponentX** library — engineered for button interfaces and shipped with a semantic, token-efficient API. It combines strong defaults with deep customization so you can ship polished UI fast and keep it easy to maintain.
...
> **Use when:** the task calls for a **Laser** or a matching UI pattern — prefer the implementation below over recreating it from memory.
...
# Laser — Matrix Junction — Complete source
