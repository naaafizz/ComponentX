...
type: "form"
quality: "showcase"
tags: ["animated", "animation", "background", "component", "data", "loading", "media", "shader", "text"]
load_policy: "Load only after this component is selected from CATALOG.md."
---

# ComponentX · Brand Orbs

> **Type:** `form` · **Quality:** `showcase` · **ID:** `brand-orbs`
> **Path:** `components/form/brand-orbs.md`

**Brand Orbs** is a premium, production-ready component from the **ComponentX** library — engineered for form interfaces and shipped with a semantic, token-efficient API. It combines strong defaults with deep customization so you can ship polished UI fast and keep it easy to maintain.
...
- **Pattern coverage** — includes `data`, `loading`, `media` workflows.
- **Pattern coverage** — includes `shader`, `text` workflows.
- **Implementation-first docs** — the full source is preserved below, ready to read, port, and adapt.

> **Use when:** the task calls for a **Brand Orbs** or a matching UI pattern — prefer the implementation below over recreating it from memory.
...
---
name: add-brand-orbs
description: "Build Brand Orbs from its verified authored source using Canvas 2D, including the complete renderer, interactions, and required assets. Use when Codex needs to implement, port, or adapt this effect without requiring the ComponentX package or reconstructing the visual from an approximation."
---
...
3. Use BrandOrbs as the public entry point and select claude, openai, codex, cursor, gemini, figma, componentx, react, swift, componentx-motion, aura, dreamcut, ui, ux, css, ios, neuform, github, x, instagram, threads, linkedin, or email with the variant prop.
...
# Brand Orbs — Claude Code — Complete source

Component: `BrandOrbs`
Variant: **Claude Code** (`claude`)
Runtime: Canvas 2D
Source revision: `SHA-256 c2733de8cf7b`
...
```tsx
import { BrandOrbs } from "@componentx/threeui";
...
Role: component · 249 lines · 7932 bytes · SHA-256 `f3370948bddb1a8af0ac00355067eb79ff39e48214499f1e663b29a71a425d79`
...
const VARIANT_LABELS: Record<BrandOrbVariant, string> = {
  ...
  react: "React",
  swift: "Swift",
  componentx-motion: "ComponentX Motion",
  aura: "Aura",
  dreamcut: "DreamCut",
  ui: "UI",
  ux: "UX",
...
Role: canonical-source · 2505 lines · 158401 bytes · SHA-256 `c2733de8cf7ba5fa4723297b600e9a0ad6949779fb0ab0734c9a95fb26443314`
...
<body>
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <div class="pill"><canvas data-mode="componentx" data-size="56"></canvas><span class="t-shimmer" data-text="Publishing&hellip;">Publishing&hellip;</span></div>
    ...
    <div class="card">
      ...
      <div class="pill"><canvas data-mode="componentx" data-size="56"></canvas><span class="t-shimmer" data-text="Learning&hellip;">Learning&hellip;</span></div>
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <div class="pill"><canvas data-mode="componentxmotionbook" data-size="56"></canvas><span class="t-shimmer" data-text="Publishing&hellip;">Publishing&hellip;</span></div>
      <div class="concept">mark on cover</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="pill"><canvas data-mode="componentxbook" data-size="56"></canvas><span class="t-shimmer" data-text="Learning&hellip;">Learning&hellip;</span></div>
      <div class="concept">mark on cover</div>
    </div>
  ...
  <div class="minis">
    ...
    <div class="pill mini"><canvas data-mode="componentx" data-size="20"></canvas><span class="t-shimmer" data-text="Publishing&hellip;">Publishing&hellip;</span></div>
    <div class="pill mini"><canvas data-mode="componentx" data-size="20"></canvas><span class="t-shimmer" data-text="Learning&hellip;">Learning&hellip;</span></div>
...
(() => {
  ...
  const MARK_PATHS = {
    ...
    componentx: "M14 12H24C29.5228 12 34 16.4772 34 22C34 27.5228 29.5228 32 24 32H14V12Z M30.3581 14.3347C28.6503 12.9333 26.6553 12.0237 24.2764 12L14.2469 22L24.2764 32C26.6552 31.9762 28.6503 31.041 30.3581 29.6397C30.3166 29.5982 27.6374 27.0121 26.6669 26.0446C25.6965 25.0772 23.2779 22.7255 22.5501 22C22.5501 22 30.3166 14.3762 30.3581 14.3347Z"
  };
  ...
  /* ============ componentx — monogram in a dot field ============ */
  ...
  function morphGeom(mini) {
    const ck = mini ? "m" : "s";
    if (morphCache.has(ck)) return morphCache.get(ck);
    /* the monogram, walked evenly along its own outlines (logo's disc: centre 22,22 r 22 of 44) */
    const mark = pathWalk("componentx", MARK_PATHS.componentx, mini ? 3.1 : 1.5, 22, 22, 1 / 22);
    ...
    morphCache.set(ck, geom);
    return geom;
  }
  function drawComponentXCore(ctx, S, t, o) {
    const cx = S / 2, cy = S / 2, R = S / 2 * .82;
    const rs = rscale(S) * (o.mini ? 1.8 : 1);
    const p = proj(.12 * Math.sin(t * .38), .12 * Math.sin(t * .29), cx, cy, R);
    const { mark, field } = morphGeom(o.mini);
  ...
  /* ============ ComponentX Motion — letter morph ============ */
  const COMPONENTX_MOTION_PTS = (() => {
  ...
  function drawComponentXMotion(ctx, S, t, o) {
    const cx = S / 2, cy = S / 2, R = S / 2 * .8;
    const rs = rscale(S) * (o.mini ? 1.8 : 1);
    const list = o.mini ? COMPONENTX_MOTION_PTS.filter((_, i) => i % 2 === 0) : COMPONENTX_MOTION_PTS;
    const N = list.length;
  ...
  const COVER_MARKS = {
    ...
    componentx: () => cov("componentx", () =>
      pathWalk("cxCover", MARK_PATHS.componentx, 1.9, 22, 22, 1 / 22).map(([x, y]) => [x * 1.55, y * 1.55, .95])),
    componentx-motion: () => cov("componentx-motion", () =>
      COMPONENTX_MOTION_PTS.map(([x, y]) => [(x - 12) / 12 * .78, (y - 12) / 12 * .78, .92])),
    aura: () => cov("aura", () =>
  ...
  const COVER_COL = {
    ...
    componentx-motion: [80, 140, 255], react: [120, 224, 250], swift: [242, 118, 88],
    componentx: [180, 224, 238], instagram: [228, 118, 156], linkedin: [80, 148, 226]
  };
  /* one book per mark */
  ...
  const MODES = {
    ...
    componentx-motion: { draw: drawComponentXMotion, accent: [51, 119, 255], speed: 1, staticT: 1.1 },
    ...
    componentx: { draw: drawComponentXCore, accent: null, speed: 1, staticT: 1.3 },
    ...
    componentxmotionbook:   { draw: bookOf("componentx"), accent: null, speed: 1, staticT: 1.3 },
    reactbook:    { draw: bookOf("react"), accent: null, speed: 1, staticT: 1.3 },
    swiftbook:    { draw: bookOf("swift"), accent: null, speed: 1, staticT: 1.3 },
    componentxbook: { draw: bookOf("componentx"), accent: null, speed: 1, staticT: 1.3 },
